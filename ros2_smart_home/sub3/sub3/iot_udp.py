...
class iot_udp(Node):
    ...
    def data_parsing(self, raw_data):
        ...
        if header == '#Appliances-Status$' and data_length[0] == 20:
            ...
            network_status = (hex(uid_pack[16]), hex(
                uid_pack[17]))  # 네트워크 상태(정상, 연결 상태, 연결 종료)
            device_status = (hex(uid_pack[18]), hex(
                uid_pack[19]))  # 디바이스 상태(ON, OFF, ERROR)
    ...
    def send_data(self, uid, cmd):
        ...
        # 4 바이트(숫자 18을 4바이트인 int형으로 바이트 type으로 바꾼다)
        data_length = pack('i', 18)
        ...
        try:
            while True:
                if len(self.parsed_data) != 0:
                    for data in self.parsed_data:
                        print('uid : ',  data[0],
                              ' ,network status: ', params_status[(
                                  int(data[1][0], 16), int(data[1][1], 16))],
                              ' ,device status : ', params_status[(
                                  int(data[2][0], 16), int(data[2][1], 16))]
                              )
                time.sleep(1)
            ...
            if params_status[(int(self.recv_data[1][0], 16), int(self.recv_data[1][1], 16))] == "CONNECTION":
                ...
                if params_status[(int(self.recv_data[2][0], 16), int(self.recv_data[2][1], 16))] == "ON":
                    while params_status[(int(self.recv_data[2][0], 16), int(self.recv_data[2][1], 16))] == "ON":
                        self.send_data(
                            self.recv_data[0], params_control_cmd["SWITCH_OFF"])
                # 꺼져있으면 킨다.
                else:
                    while params_status[(int(self.recv_data[2][0], 16), int(self.recv_data[2][1], 16))] == "OFF":
                        self.send_data(
                            self.recv_data[0], params_control_cmd["SWITCH_ON"])
        '''
        로직 8. iot control
