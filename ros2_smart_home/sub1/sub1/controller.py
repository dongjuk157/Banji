...
class Controller(Node):
    ...
    def __init__(self):
        ...
        self.app_control_pub = self.create_publisher(
            Int8MultiArray, 'app_control', 10)
    ...
    def app_off_select(self, num):
        ...
        로직 3. 특정 가전 제품 OFF
        '''
        self.app_control_msg.data[num] = 0
        self.app_control_pub.publish(self.app_control_msg)
    ...
    def timer_callback(self):
        ...
        print(
            '##############################################################################')
        print('터틀 봇 상태')
        print(self.turtlebot_status_msg, '마무리')
        print('twist.linear.x -> ', self.turtlebot_status_msg.twist.linear.x)
        print('twist.angular.z -> ', self.turtlebot_status_msg.twist.angular.z)
        print('배터리 용량 -> ', self.turtlebot_status_msg.battery_percentage,
              '파워 공급 상태 -> ', self.turtlebot_status_msg.power_supply_status)
        print(
            '##############################################################################')
        print('환경')
        print('날짜 -> ', self.envir_status_msg.month,
              '월 ', self.envir_status_msg.day, '일')
        print('시간 -> ', self.envir_status_msg.hour,
              '시 ', self.envir_status_msg.minute, '분')
        print('기온 -> ', self.envir_status_msg.temperature, '도')
        print('날씨 -> ', self.envir_status_msg.weather)
        print(
            '##############################################################################')
